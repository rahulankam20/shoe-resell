...
export default function AdminPage() {
  ...
  return <div className="admin-page page-shell"><header className="admin-head"><div><p className="eyebrow accent">SOLEVAULT OPERATIONS</p><h1>ADMIN CONSOLE.</h1></div><button className="button outline" onClick={load}><RefreshCcw /> Refresh</button></header><nav className="admin-tabs">{([['products', PackageSearch, 'Products'], ['orders', ShoppingBag, 'Orders'], ['payments', CreditCard, 'Payments'], ['taxonomy', Tags, 'Brands & categories'], ['users', Users, 'Users']] as const).map(([key, Icon, label]) => <button className={tab === key ? 'active' : ''} onClick={() => setTab(key)} key={key}><Icon />{label}</button>)}</nav>{loading ? <AdminPageSkeleton /> : error ? <ErrorState message={error} retry={load} /> : <section className="admin-panel">
